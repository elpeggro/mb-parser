...
ReferenceGraph::ReferenceGraph(std::vector<Frame> frames_, std::vector<Macroblock> mbs_)
    : frames(std::move(frames_)), mbs(std::move(mbs_)), max_weight(0) {
...
void ReferenceGraph::printAsDot(const std::string &path) {
  ...
  for (auto &frame_it : graph) {
    for (auto weight_it = frame_it.second.begin(); weight_it != frame_it.second.end(); weight_it++) {
      ...
      out << weight_it->first << " -> " << frame_it.first << " [label=\"" << weight_it->second << "\",penwidth=\""
          << edge_weight << "\"];\n";
    }
  }
  out << "}";
